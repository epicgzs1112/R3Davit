...
class LGAtention(Attention):
    ...
    def forward(self, x, view_num=None, context=None, mask=None, lga=False):
        print(f'\n x in lga_atten:{x.shape} ')#16 196 768
        b, n, _ = x.shape  
        kv_input = default(context, x)
        print(f'\n kv input|:{kv_input.shape}')#16 196 768
        q_input = x

        q = self.to_q(q_input)#      self.to_q = torch.nn.Linear(dim, dim, bias=qkv_bias) dim=768

        k = self.to_k(kv_input)# self.to_k = torch.nn.Linear(dim, dim, bias=qkv_bias)

        v = self.to_v(kv_input)# self.to_v = torch.nn.Linear(dim, dim, bias=qkv_bias)
        #print(f'\n q k v before map q.shape:{q.shape},k.shape:{k.shape},v.shape:{v.shape}')
        # [B, P, D]
        q, k, v = map(lambda t: rearrange(t, 'b n (h d) -> b h n d', h=self.num_heads), (q, k, v))#h=12
        # [B, H, P, d]
        print(f'\n q k v after map q.shape:{q.shape},k.shape:{k.shape},v.shape:{v.shape}')
        #q k v after map q.shape:torch.Size([16, 12, 196, 64]),k.shape:torch.Size([16, 12, 196, 64]),v.shape:torch.Size([16, 12, 196, 64])
        if lga is True:
            #print(f'\n q k v in lga  q.shape:{q.shape},k.shape:{k.shape},v.shape:{v.shape}')
            b, _, _, _ = q.shape
            b_s = int(b // view_num)
            #print(f'\n b_s:{b_s}')
            token_num = 14 // self.group_num#7
            print(f'\n b_S:{b_s}view_num:{view_num}token_num:{token_num}group_num:{self.group_num}')
            #q :16, 12, 196, 64
            ...
            #print(f'\n q_cls:{q_cls.shape}k_cls:{k_cls.shape}v_cls:{v_cls.shape}')
            ...
            #print(f'\n out after rearange :{out.shape}')
            #print(f'\n v before IVS :{v.shape}')
            ifs = self.InterViewSignature(v)# self.IFS = torch.nn.Conv2d(self.dim, self.dim, kernel_size=3, padding=1, groups=self.dim)
            #print(f'\n v after IVS :{v.shape}')
            out = out + ifs
        else:
            #print(f'\n calcu  attention  q.shape:{q.shape},k.shape:{k.shape},v.shape:{v.shape}')
            dots = torch.einsum('b h i d, b h j d -> b h i j', q, k) * self.scale
            #print(f'\n dots:{dots.shape}')
            if mask is not None:
                dots = dots + mask
            attn = dots.softmax(dim=-1)  # [B, H, P_q, P_kv]
            #print(f'\n dots after softmax :{dots.shape}')
            attn = self.attn_drop(attn)

            #print(f'\n attn:{attn.shape},value:{v.shape}')
            out = torch.einsum('b h i j, b h j d -> b h i d', attn, v)  # [B, H, P_q, d]
        #print(f'\n out before rearrange:{out.shape}')# out before rearrange:torch.Size([16, 12, 196, 64])
        out = rearrange(out, 'b h n d -> b n (h d)')  # [B, P_q, D]
        #print(f'\n out after rearrange:{out.shape}')  # out before rearrange:torch.Size([16, 196, 768])
        out = self.proj(out)# self.proj = torch.nn.Linear(dim, dim) dim=768
        out = self.proj_drop(out)
       # print(f'\n atten out :{out.shape}')#16 196 768
        return out
