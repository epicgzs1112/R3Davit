...
class Block(torch.nn.Module):
    ...
    def forward(self, x, view_num=None,lga=False):
        #print(f'\n lga :{lga}')
        attn = self.attn(self.norm1(x), view_num=view_num,lga=False)
...
class STMBlock(torch.nn.Module):
    ...
    def cluster(self, feature, batch_size):
        #print(f'\n in cluster :feature{feature.shape} batch_size:{batch_size}')
        with torch.no_grad():
            distance_matrix = torch.cdist(feature, feature) / (self.dim ** 0.5)
         #   print(f'\n distance _matrix:{distance_matrix.shape},self.dim**0.5:{self.dim**0.5}root of 768')
            # get local density
            distance_nearest, index_nearest = \
                torch.topk(distance_matrix, k=self.k, dim=-1, largest=False)
          #  print(f'\n after topk distance_nearest:{distance_nearest.shape} index_nearest:{index_nearest.shape}')
           ...
           # print(f'\n density:{density.shape}')
            ...
            #print(f'\n density[:, None, :] :{density[:, None, :].shape} density[:, :, None]:{density[:, :, None].shape}')
            #print(f'\n mask:{mask.shape}')
            mask = mask.type(feature.dtype)
            #print(f'\n distance_matrix:{distance_matrix.flatten(1).shape}')# distance_matrix:torch.Size([16, 38416])


            dist_max = distance_matrix.flatten(1).max(dim=-1)[0][:, None, None]
            #print(f'\n dis_max:{dist_max.shape}')
            dist, index_parent = (distance_matrix * mask + dist_max * (1 - mask)).min(dim=-1)
            #print(f'\n dist:{dist.shape} index_parent:{index_parent.shape}')
    ...
    def merger(self, feature, idx_cluster, token_score, batch_size, patch_num):
        ...
        #print(f'\n idx_cluster:{idx_cluster.shape}')
        #print(f'\n idx_batch:{idx_batch.shape}')
        #print(f'\n out_token_len:{self.out_token_len}')

        idx = idx_cluster + idx_batch * self.out_token_len  
        #print(f'\n idx:{idx.shape}')
        token_weight = token_score.exp()
        #print(f'\n token_score:{token_score.shape}')
