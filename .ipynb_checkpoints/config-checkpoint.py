...
__C.DATASETS.SHAPENET.RENDERING_PATH        = '/root/autodl-tmp/R3DSWIN++/ShapeNetRendering/%s/%s/rendering/%02d.png'
# __C.DATASETS.SHAPENET.RENDERING_PATH      = '/home/hzxie/Datasets/ShapeNet/PascalShapeNetRendering/%s/%s/render_%04d.jpg'
__C.DATASETS.SHAPENET.VOXEL_PATH            = '/root/autodl-tmp/R3DSWIN++/ShapeNetVox32/%s/%s/model.binvox'
...
__C.NETWORK.DECODER.GROUP.HEADS = 8
__C.NETWORK.DECODER.GROUP.DIM = 784
cfg.NETWORK.DECODER.VOXEL_SIZE=32
# Merger (STM)
__C.NETWORK.MERGER = edict()
__C.NETWORK.MERGER.FC = edict()
__C.NETWORK.MERGER.STM = edict()
__C.NETWORK.MERGER.STM.DIM = 1024
__C.NETWORK.MERGER.STM.OUT_TOKEN_LENS = [49, 49]
__C.NETWORK.MERGER.STM.K = 5
__C.NETWORK.MERGER.STM.NUM_HEAD = 8
__C.NETWORK.MERGER.FC.DIM=1024
# Training
__C.TRAIN = edict()
