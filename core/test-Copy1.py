...
def test_net(cfg,
    ...
    for sample_idx, (taxonomy_id, sample_name, rendering_images, ground_truth_volume) in for_tqdm:
        ...
        with torch.no_grad():
            ...
            if not os.path.exists('/home/lch/Downloads/Long-Range-Grouping-Transformer-main/output/%s' % (taxonomy_id)):
                os.makedirs('/home/lch/Downloads/Long-Range-Grouping-Transformer-main/output/%s' % (taxonomy_id))
            with open('/home/lch/Downloads/Long-Range-Grouping-Transformer-main/output/%s/%s.binvox' % (taxonomy_id, sample_name),
                      'wb') as f:
