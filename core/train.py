...
def train_net(cfg):
    ...
    init_epoch, best_iou, best_epoch, encoder, decoder, cfg = \
        pipeline.setup_network(cfg, encoder, decoder)

    # Set up solver
    encoder_solver, decoder_solver, = pipeline.solver(cfg, encoder, decoder)
    ...
    for epoch_idx in range(init_epoch, cfg.TRAIN.NUM_EPOCHS):
        ...
        iou = test_net(cfg, epoch_idx + 1, val_data_loader, val_file_num, val_writer, encoder, decoder)
