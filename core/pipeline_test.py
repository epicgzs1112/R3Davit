...
def setup_network(cfg, encoder, decoder):
    ...
    return encoder, decoder, epoch_idx
...
def combine_test_f1(testf1, taxonomies_list, taxonomies, test_file_num):
    world_size = 1
    all_test_f1 = [torch.zeros_like(testf1) for _ in range(world_size)]
    all_taxonomies_list = [torch.zeros_like(taxonomies_list) for _ in range(world_size)]
    torch.distributed.all_gather(all_test_f1, testf1)
    torch.distributed.all_gather(all_taxonomies_list, taxonomies_list)
    if torch.distributed.get_rank() == 0:
        redundancy = test_file_num % world_size
        if redundancy == 0:
            redundancy = world_size
        for i in range(world_size):
            all_test_f1[i] = all_test_f1[i] \
                if i < redundancy else all_test_f1[i][:-1, :]
            all_taxonomies_list[i] = all_taxonomies_list[i] \
                if i < redundancy else all_taxonomies_list[i][:-1]
        all_test_f1 = torch.cat(all_test_f1, dim=0).cpu().numpy()  # [sample_num, 4]
        all_taxonomies_list = torch.cat(all_taxonomies_list).cpu().numpy()  # [sample_num]
        test_f1 = dict()
        for taxonomy_id, sample_f1 in zip(all_taxonomies_list, all_test_f1):
            if taxonomies[taxonomy_id] not in test_f1.keys():
                test_f1[taxonomies[taxonomy_id]] = {'n_samples': 0, 'f1': []}
            test_f1[taxonomies[taxonomy_id]]['n_samples'] += 1
            test_f1[taxonomies[taxonomy_id]]['f1'].append(sample_f1)
        return test_f1
    else:
        return
...
def output(cfg, test_iou, taxonomies):
    ...
    return mean_iou
def output2(cfg, test_f1, taxonomies):
    mean_f1 = []
    n_samples = 0
    for taxonomy_id in test_f1:
        test_f1[taxonomy_id]['f1'] = np.mean(test_f1[taxonomy_id]['f1'], axis=0)
        mean_f1.append(test_f1[taxonomy_id]['f1'] * test_f1[taxonomy_id]['n_samples'])
        n_samples += test_f1[taxonomy_id]['n_samples']
    mean_f1 = np.sum(mean_f1, axis=0) / n_samples
    ...
    for taxonomy_id in test_f1:
        print('%s' % taxonomies[taxonomy_id]['taxonomy_name'].ljust(8), end='\t')
        print('%d' % test_f1[taxonomy_id]['n_samples'], end='\t')
        print('N/a', end='\t\t')

        for ti in test_f1[taxonomy_id]['f1']:
            print('%.4f' % ti, end='\t')
        print()
