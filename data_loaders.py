...
class Pix3dDataset(torch.utils.data.dataset.Dataset):
    ...
    def get_datum(self, idx):
        ...
        if len(rendering_image.shape) < 3:
            print('[WARN] %s It seems the image file %s is grayscale.' % (dt.datetime.now(), rendering_image_path))
...
class Pix3dDataLoader:
    ...
    def get_dataset(self, dataset_type, n_views_rendering, transforms=None):
        ...
        for taxonomy in self.dataset_taxonomy:
            ...
            print('[INFO] %s Collecting files of Taxonomy[Name=%s]' % (dt.datetime.now(), taxonomy_name))
        ...
        print('[INFO] %s Complete collecting files of the dataset. Total files: %d.' % (dt.datetime.now(), len(files)))
        ...
        for sample_idx, sample_name in enumerate(samples):
            ...
            if not os.path.exists(volume_file_path):
                print('[WARN] %s Ignore sample %s/%s since volume file not exists.' %
                      (dt.datetime.now(), taxonomy_name, sample_name))
                continue
